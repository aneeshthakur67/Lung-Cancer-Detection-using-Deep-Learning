...
def add_lungs_pred(request):
    ...
    if request.method == "POST":
        ...
        patient = Patient.objects.get(user=request.user)
        data = Search_Data.objects.create(patient=patient, prediction_accuracy=accuracy, result=clas_name,
                                          predict_for="Lungs Cancer Prediction", uploaded=file, output=output)
